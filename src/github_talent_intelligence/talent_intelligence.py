...
class TalentAnalyzer:
    ...
    def _get_github_token(self) -> str:
        ...
        raise ValueError("GitHub token not found. Set GITHUB_TOKEN environment variable or use GitHub CLI.")
    ...
    def analyze_repositories(self, repos_data: List[Dict], **kwargs) -> List[Repository]:
        """
        ...
        classify_roles = kwargs.get('classify_roles', self.config['analysis']['classify_roles'])
        ...
        for repo_data in tqdm(repos_data, desc="Analyzing repositories"):
            try:
                ...
                for contrib_data in contributors_data[:self.config['analysis']['max_contributors_per_repo']]:
                    ...
                    filtered_contrib = {k: v for k, v in contrib_data.items() if k in valid_keys}
            ...
            'contributors': [
                {
                    ...
                    'repositories': [
                        ...
                        for repo in repositories
                        if any(contrib.login == c.login for contrib in repo.contributors)
                    ]
                }
    ...
    def save_results(self, repositories: List[Repository], output_dir: str, 
    ...
    def _get_indicators(self, repo: Dict) -> List[str]:
        ...
        indicators = ['infra', 'node', 'validator', 'iac', 'docker', 'ops', 'deployment', 
                     'k8s', 'kubernetes', 'ci', 'cd', 'devops', 'compose', 'ansible', 
                     'terraform', 'helm', 'cluster', 'monitor', 'prometheus', 'grafana', 
                     'pipeline', 'automation', 'build', 'test', 'runner', 'orchestrator']
    ...
    def _save_csv(self, repositories: List[Repository], output_dir: str):
        ...
        repo_columns = [
            'full_name','name','classification','private','html_url','description','fork','url',
            'created_at','updated_at','pushed_at','git_url','ssh_url','clone_url','homepage','size',
            'stargazers_count','watchers_count','forks_count','open_issues_count','language','license',
            'owner_login','indicators'
        ]
        # Reference contributor columns
        contrib_columns = [
            'repo_full_name','login','id','html_url','type','site_admin','contributions','name','company',
            'blog','location','email','hireable','bio','twitter_username','public_repos','public_gists',
            'followers','following','roles'
        ]
        ...
        with open(repo_path, 'w', newline='') as f:
            ...
            for repo in repositories:
                ...
                row['indicators'] = ','.join(repo.indicators) if repo.indicators else api.get('indicators', '')
