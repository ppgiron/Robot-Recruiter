...
@click.option('--repos', help='Comma-separated list of repositories (owner/repo format)')
@click.option('--output', default='results', help='Output directory for results')
@click.option('--formats', default='json,csv,recruiting', help='Output formats (comma-separated)')
@click.option('--use-nlp/--no-nlp', default=True, help='Use NLP classification')
@click.option('--classify-roles/--no-classify-roles', default=True, help='Classify contributor roles')
...
def analyze(org, user, repos, output, formats, use_nlp, classify_roles, config):
    ...
    try:
        ...
        click.echo(f"Total contributors: {sum(len(r.contributors) for r in repositories)}")
...
def contributors(repo_full_name, output):
    ...
    try:
        ...
        with open(output, 'w') as f:
            json.dump([analyzer._contributor_to_dict(c) for c in contributors], f, indent=2, default=str)
        ...
        for i, contrib in enumerate(top_contributors, 1):
            click.echo(f"  {i}. {contrib.login} ({contrib.contributions} contributions)")
...
def export_recruiting(input_file, output):
    ...
    try:
        ...
        from talent_intelligence import Repository, Contributor
        
        repositories = []
        for repo_data in data:
            contributors = [Contributor(**c) for c in repo_data['contributors']]
            repo = Repository(**{k: v for k, v in repo_data.items() if k != 'contributors'})
